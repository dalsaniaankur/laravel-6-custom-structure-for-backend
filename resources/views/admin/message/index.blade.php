...
@section('content')
            ...
            <div class="page-wrap bx-shadow mt-5 mb-5">
                <div class="row user-dt-wrap">
                    <div class="col-lg-12 col-md-12 pb-5">
                        <h1 class="admin_bigheading mb-5 inlinebtn">Messages
                            @if($userId > 0 && !empty($userDropDown) && count($userDropDown) > 0 && isset($userDropDown->toArray()[$userId]))
                                for {{ strtolower($userDropDown->toArray()[$userId]) }}
                            @endif
                            ({{ $totalRecordCount }})</h1>
                        ...
                        {!! Form::open(['method' => 'GET','name'=>'admin-message-search', 'id' =>'admin_message_search_form', 'class'=>'top-search-options form-row pb-4','route' => ['admin.messages']]) !!}
                        ...
                        <div class="col-md-3 col-lg-4">
                            {!! Form::select('role_id', ['' => 'User type','2' => 'School','3' => 'Teacher','4' => 'Student','5' => 'Parent','6' => 'PTA Member'], $roleId, ['onchange' => "getUserDropDownByRoleType(this,'#user_id_for_search')",'class' => 'form-control mb-3']) !!}
                        </div>

                        <div class="col-md-4 col-lg-3">
                            {!! Form::select('user_id', $userDropDown, $userId, ['class' => 'form-control mb-3', 'id' => 'user_id_for_search']) !!}
                        </div>

                        <div class="col-md-3 col-lg-2 offset-lg-7">
                        ...
                        <ul class="nav nav-tabs mt-5">
                            <li class="nav-item">
                                <a class="received-tab nav-link {{ $currentTab =='received' ? 'active' : '' }}" href="#received" data-toggle="tab">Received</a></li>
                            <li class="nav-item">
                                <a class="sent-tab nav-link {{ $currentTab =='sent' ? 'active' : '' }}" href="#sent" data-toggle="tab">Sent</a>
                            </li>
                        </ul>
                                        ...
                                        @if(!empty($messages) && count($messages) > 0)
                                            @foreach($messages as $messageKey => $message)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div
                                                            class="elips">{{ $message->getLastMessage($message->sender_id,$message->receiver_id)->message }}</div>
                                                    </td>
    ...
    @include('admin.message.add_edit_model')
...
@section('javascript')
    <script>
        ...
        window.addEditMessageUrl = "{{ URL::to('admin/message/save_ajax') }}";
        window.getMessageDataForEditUrl = "{{ URL::to('admin/message/get_data') }}";
        window.getUserDropDownUrl = "{{ URL::to('getuserdropdown') }}";

        addEditMessageModelIdElement = $(addEditMessageForm + " #add_edit_message_model_id");
        roleIdElement = $(addEditMessageForm + " .role_id");
        ...
        /* User Drop Down */
        function getUserDropDownForNotification() {
            var roleId = roleIdElement.val();
            showLoader();
            jQuery.ajax({
                url: window.getUserDropDownUrl,
                method: 'post',
                dataType: 'JSON',
                data: {
                ...
                success: function (response) {
                    if (response.success == true) {
                        ...
                        receiverIdElement.find('option').remove();
                        jQuery.each(DropDownData, function (key, value) {
                            if (value != "" && key != "") {
                                receiverIdElement.append($('<option></option>').val(value).html(key));
                            }
                        });
                        receiverIdElement.multiselect('reload');
                    }
                    hideLoader();
                },
