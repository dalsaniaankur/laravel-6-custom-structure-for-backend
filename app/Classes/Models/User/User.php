...
class User extends AuthBaseModel
{
    ...
    public function getDateOfBirthDatepickerFormatAttribute()
    {
        if ( ! empty( $this->date_of_birth ) && $this->date_of_birth != '0000-00-00' ) {
            return \DateFacades::dateFormat( $this->date_of_birth, 'format-datepicker' );
        }
        return '';
    }
    ...
    public function getSchoolDropDownFlip( $prepend = '', $status = 1, $prependKey = 0 )
    {
        $roleId = $this->_helperRoles->getSchoolRoleId();
        $return = $this->setSelect()
                       ->addRoleIdFilter( $roleId )
                       ...
                       ->addSortOrder( ['school_name' => 'asc'] )
                       ->get()
                       ->pluck( 'user_id', 'school_name' );

        if ( ! empty( $prepend ) ) {
            $return->prepend( $prependKey, $prepend );
        }
        return $return;
    }

    public function getDropDown( $prepend = '', $roleId = 0, $schoolId = 0, $status = -1, $isVerified = -1, $classId = 0, $userIdNotIn = [], $prependValue = 0 )
    {
        $return = $this->setSelect()
                       ->addStatusFilter('status' ,$status )
        ...
        if ( ! empty( $prepend ) ) {
            $return->prepend( $prepend, $prependValue );
        }
        return $return;
    }
    ...
    public function joinStudentParent( $searchable = false )
    {
        $studentParentObj = new StudentParent();
        $studentParentTableName = $studentParentObj->getTable() . ' as student_parent';
        $searchableColumns = $studentParentObj->getSearchableColumns();
        $this->joinTables[] = ['table'             => $studentParentTableName,
                               'searchable'        => $searchable,
                               'searchableColumns' => $searchableColumns];
        $this->queryBuilder->leftJoin( $studentParentTableName, function ( $join ) use ( $studentParentTableName ) {
            $join->on( $this->table . '.user_id', '=', 'student_parent.student_id' );
    ...
    public function addParentIdFilter( $parentId = 0 )
    {
        if ( ! empty( $parentId ) && $parentId > 0 ) {
            $studentParentObj = new StudentParent();
            $studentParentTableName = 'student_parent';
            $this->queryBuilder->where( $studentParentTableName . '.parent_id', '=', $parentId );
        }
        return $this;
    }
    ...
    public function getList( $searchHelper )
    {
        ...
        $parentId = ( ! empty( $searchHelper->_filter['parent_id'] )) ? $searchHelper->_filter['parent_id'] : -1;
    ...
    public function addJoinsAndFilters( $searchHelper )
    {
        ...
        $this->leftJoinModel( $this->studentClubObj, 'user_id','user_id','student_club', $searchable = false )
             ->addSearch( $search )
             ->addEqualIdFilter( 'role_id', $roleId )
             ->addEqualStringFilter( 'email', $email )
             ->addInIdFilter( 'role_id', [] )
             ->addStatusFilter( 'status', 1 );
        return $this;
