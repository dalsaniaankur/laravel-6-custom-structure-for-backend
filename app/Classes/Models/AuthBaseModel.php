...
class AuthBaseModel extends Authenticatable implements MustVerifyEmail
{
    ...
    public function removed( $id )
    {
        ...
        if ( ! empty( $deleteObj ) ) {
            ...
            /* Delete Files */
            $fileFields = $deleteObj->getFileField();
            foreach ($fileFields as $fileFieldKey => $fileField){
                if ( ! empty( $deleteObj->{$fileField} ) ) {
                    Common::deleteFile( $deleteObj->{$fileField} );
                }
            }
    ...
    $filter = ['prepend_key' => '','prepend_value' => 'Select User','key_field' => 'user_id','value_field' => 'name'];
    $searchHelperForGradeDropDown = new SearchHelper( -1, -1, ['*'] , $filter,['first_name' => 'asc'] );
    ...
    public function leftJoinModel( $modelObj, $currentModelKey='', $joinModelKey='', $tableAlias = '', $searchable = false )
    {
    ...
    public function joinMode( $modelObj, $currentModelKey='', $joinModelKey='', $tableAlias = '', $searchable = false )
    {
        $tableName = $modelObj->getTableName();
        if(!empty($tableAlias)){
            $tableName = $tableName . ' as '.$tableAlias;
