...
class CommonController extends Controller
{
    ...
    {
        $data = $request->all();
        $dropDown = $this->clubObj->getDropDown();
    ...
    public function getUserDropdownByClubIdForNotification( Request $request )
    {
        ...
        $filter = ['role_id_in'     => [$studentRoleId,$teacherRoleId],
                   'club_id'     => $clubId,
                   'status'      => 1,
                   'is_verified' => 1,
                   'is_fcm_token' => 1];
    ...
    public function getGradeDropdownForNotification( Request $request )
    {
        $data = $request->all();
        $filter = [];
        $searchHelper = new SearchHelper( -1, -1, $selectColumns = ['grade_id','grade_name'], $filter,['grade_name' =>'asc'] );
        $gradeDropDown = $this->gradeObj->getList( $searchHelper )->pluck( 'grade_name', 'grade_id' );
        return response()->json( $gradeDropDown );
    }
