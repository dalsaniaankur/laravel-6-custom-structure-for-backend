...
class IndexController extends Controller
{
    ...
    public function index( Request $request )
    {
        ...
        $receiverId = Auth::guard('teacher')->user()->user_id;
        ...
        if ( $roleId > 0 ) {
            ...
            if ( $roleId == $schoolRoleId ) {
                $userDropDown = $this->userObj->getSchoolDropDown( 'Users', $status = 1, $prependKey = 0 );
            } else {
        ...
        return view( 'teacher.message.index', compact( 'sortedBy', 'sortedOrder', 'recordStart', 'messages', 'paging', 'totalRecordCount', 'createdStartDate', 'createdEndDate','schoolId','roleId', 'currentTab', 'messagesForSent', 'totalRecordCountSent', 'pagingForSent', 'recordStartForSent' ) );
    }
	...
	public function saveAjax( Request $request )
    {
        $data = $request->all();
        $data['status'] = 1;
        $data['sender_id'] = Auth::guard( 'teacher' )->user()->user_id;
        ...
        foreach ( $receiverIdArray as $receiverIdKey => $receiverId ) {
            ...
            if ( ! empty( $results['message_id'] ) && $results['message_id'] > 0 ) {
                ...
                $senderName = Auth::guard( 'teacher' )->user()->name;
