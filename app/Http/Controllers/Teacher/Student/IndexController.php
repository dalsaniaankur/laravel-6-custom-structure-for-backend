...
class IndexController extends Controller
{
    ...
    {
        $data = $request->all();
		$teacherInfo = Auth::guard('teacher')->user();
        ...
        $gradeDropDown = $this->gradeObj->getDropDown( $prepend = 'Grades', $schoolId,'' );
        ...
        $schoolDropDown = $this->userObj->getDropDown($prepend = '', $schoolRoleId, 0, 1, 1);
        $classesDropDown = [];
        ...
        return view( 'teacher.student.index', compact( 'schoolId', 'sortedBy', 'sortedOrder', 'recordStart', 'students', 'paging', 'totalRecordCount', 'name', 'email', 'statusDropDown', 'status', 'createdStartDate', 'createdEndDate', 'gender', 'gradeDropDown', 'clubDropDown', 'allergyDropDown', 'clubId', 'allergieId', 'gradeId','schoolDropDown','classesDropDown','clubDropDownForAddEdit','allergyDropDownForAddEdit','school','parentId','parentDropDown' ) );
    }
    ...
    public function saveAjax( Request $request )
    {
        ...
        $data['class_id']  = $teacherInfo->class_id;
        $data['grade_id']  = $teacherInfo->grade_id;
    ...
    public function profileDetails(Request $request, $studentId){
        ...
        $schoolRoleId = $this->_helperRoles->getSchoolRoleId();
        $schoolDropDown = $this->userObj->getDropDown('', $schoolRoleId, 0, 1, 1);
        $classesDropDown = $this->classesObj->getDropDown( $student->school_id, 1, $student->grade_id );
        ...
        return view('teacher.student.profile',compact('student','gradeDropDown','allergyDropDown','allergies','clubs','clubDropDown','studentParents','schoolDropDown','classesDropDown','parentDropDown','parentId','studentParentId'));
    }
    ...
    public function academicsDetails(Request $request, $studentId){
        ...
        return view( 'teacher.student.academics', compact(  'sortedOrder', 'exams','student', 'studentId', 'examsList' ) );
    }
	...
	public function examResultSave(Request $request)	{
		...
		foreach( $examLists as $examKey => $examList ) {
			...
			if( !empty($examList['subject']) && !empty( $examList['percentage'] ) ) {
				...
				$examResult['created_user_id'] = Auth::guard('teacher')->getUser()->user_id;
